...
float trapezium(float x)
{
    ...
    //
	return min(1.0, max(0.0, 1.0 - abs(-fmod(x, 1.0) * 3.0 + 1.0)) * 2.0);
}

float3 colFromHue(float hue)
{
    // https://en.wikipedia.org/wiki/Hue#/media/File:HSV-RGB-comparison.svg
	return float3(trapezium(hue - 1.0/3.0), trapezium(hue), trapezium(hue + 1.0/3.0));
}
...
float getPopRandom(float time, float random) {
    ...
    return rand(float2(time, random));
}

// Gets a random value for the most recent explosion at the specified time.
float getPopTime(float time, float iTime) {
...
float isInExtendedTriangle(float2 b, float2 a, float2 c, float2 p)
{
    // Compute vectors        
    float2 v0 = c - a;
    float2 v1 = b - a;
...
float isInQuad(float2 a, float2 b, float2 c, float2 d, float2 p)
{
    return isInExtendedTriangle(a, b, c, p) * isInExtendedTriangle(c, d, a, p);
}

float4 rotate(float phi, float theta, float psi) {
    float cosPhi = cos(phi), sinPhi = sin(phi);
    float cosTheta = cos(theta), sinTheta = sin(theta);
    float cosPsi = cos(psi), sinPsi = sin(psi);
    
    float3 row0 = float3(cosTheta * cosPsi, -cosTheta * sinPsi, sinTheta);
    
    float3 row1 = float3(cosPhi * sinPsi + sinPhi * sinTheta * cosPsi, 
...
float isInRotatedQuad(float4 offsets, float2 center, float2 p)
{
...
void confetti_float(float2 uv, float time,  out float4 Confetti)
{
    float2 scaledFragCoord = uv;
    ...
    float4 matrix1 = rotate(t * 8.0, sin(t) * 0.5, t / 4.0) * size;
    t = time * confettiRotateTimeScale / 5.1 + 92.2;
    float4 matrix2 = rotate(t * 8.0, sin(t) * 0.5, t / 4.0) * size;
    t = time * confettiRotateTimeScale / 5.5 + 7.1;
    float4 matrix3 = rotate(t * 8.0, sin(t) * 0.5, t / 4.0) * size;
    t = time * confettiRotateTimeScale / 4.3 + 1.0;
    float4 matrix4 = rotate(t * 8.0, sin(t) * 0.5, t / 4.0) * size;
    
    float4 col = float4(0.0, 0.0, 0.0, 0.0);
    for (int i=0; i<N_POPS; i++) {
        float sampleTime = time - float(i) * TIME_BETWEEN_POPS;
        float popTime = getPopTime(sampleTime, time);
        float2 pointPop = float2(getPopRandom(sampleTime, 3.1) * 800.0, 0);
        float baseHue = getPopRandom(sampleTime, 3.5);
        
        if (abs(scaledFragCoord.x - pointPop.x) > 300.0) {
            continue;
        }
        ...
        for (int j=0; j<N_CONFETTI; j++) {
            ...
            float2 confettiLocation = float2(pointPop.x + x, y);
            float2 delta = confettiLocation - scaledFragCoord;
            ...
            float4 matrixVal = float4(0., 0., 0., 0.);
            float matrixIndex = fmod(f * 4.0, 4.0);
            if (matrixIndex < 1.0) {
                matrixVal = matrix1;
            ...
            float squareIntensity = isInRotatedQuad(matrixVal,
                                  			        confettiLocation, 
                                                    scaledFragCoord);
            float3 pastelColour = colFromHue(baseHue + getPopRandom(sampleTime, float(j - 15)) * HUE_VARIANCE);
            float3 pastelGlowColour = glowIntensity * (pastelColour * (1.0 - GLOW_WHITENESS) + float3(GLOW_WHITENESS, GLOW_WHITENESS, GLOW_WHITENESS));
            float3 pastelSqColour = squareIntensity * (pastelColour * (1.0 - SQUARE_WHITENESS) + float3(SQUARE_WHITENESS, SQUARE_WHITENESS, SQUARE_WHITENESS));
            col += float4(pastelGlowColour + pastelSqColour, 0.0);
        }
    }
    col.a = 1.0;
